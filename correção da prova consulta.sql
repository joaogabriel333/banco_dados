...
INSERT INTO funcionarios_projetos (id_funcionario,id_projeto,horas_trabalhadas,valor_hora) VALUES
	...
	-- 1
	select count(distinct fp.id_projeto) as total_projetos
...
-- 2
select p.nome as projeto, d.nome as departamento, p.valor_total
from projetos p
...
-- 3 
select f.nome as funcionario, avg(fp.horas_trabalhadas) as media 
from funcionarios f
inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario
group by f.nome;
...
-- 8
select avg(p.valor_total) as media
from projetos p 
...
-- 12
select p.nome as projeto, f.nome  as funcionario, sum(fp.horas_trabalhadas) as horas_trabalhada
from projetos p
inner join funcionarios_projetos fp on p.id_projeto = fp.id_projeto 
